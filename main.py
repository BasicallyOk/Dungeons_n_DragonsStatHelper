...
async def on_message(message):
    ...
    if "character choice" in content:
        ...
        try:
            members[message.author.name] = player(items[statindex + 1], finalRole[message.author], int(items[statindex + 2]), items[statindex + 3], int(items[statindex + 4]), int(items[statindex + 5]), int(items[statindex + 6]), int(items[statindex + 7]), int(items[statindex + 8]), int(items[statindex + 9]))
